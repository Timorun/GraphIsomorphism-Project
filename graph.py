...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        # intialize new graph, and go through each vertex in selfgraph and add to new graph same with graph2
        G = Graph(False)
        selfdict = dict()
        for vertex in self.vertices:
            newvertex = Vertex(G)
            selfdict[vertex] = newvertex
            G.add_vertex(newvertex)
        for edge in self.edges:
            newvertex1 = selfdict[edge.head]
            newvertex2 = selfdict[edge.tail]
